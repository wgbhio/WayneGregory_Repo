...
# --- BEGIN: Excluded Domain Controllers ---
$ExcludedDCs = @(
    "azcusdc002.bfl.local",
    "azeus2dc001.bfl.local",
    "azeus2dc002.bfl.local",
    "azeus2dc004.bfl.local",
    "azeus2dc003.bfl.local",
    "azneudc001.bfl.local",
    "azweudc003.bfl.local",
    "azweudc002.bfl.local",
    "azweudc004.bfl.local",
    "azweudc001.bfl.local",
    "azneudc002.bfl.local"
)
# --- END: Excluded Domain Controllers ---
        ...
        function HammerDC
            {
                ...
                #start-job -Name ($DC.Name+'_CleartxtEvt') -scriptblock {(Get-EventLog -LogName Security -InstanceId 4624 -Message '*Logon Type:			8*' -ComputerName $args).Count} -ArgumentList $DC.Name | Out-Null
                    ...
                    $Inv = ([PowerShell]::Create()).AddScript({param($DomControl)
...
Get-ADDomainController -Server $DomControl}).AddArgument($($args[0]))

                    $Software64 = ([PowerShell]::Create()).AddScript({param($DomControl)
...
Invoke-Command -cn $DomControl -ScriptBlock {Get-ItemProperty HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\*}}).AddArgument($($args[0]))

                    $Software86 = ([PowerShell]::Create()).AddScript({param($DomControl)
...
Invoke-Command -cn $DomControl -ScriptBlock {Get-ItemProperty HKLM:\Software\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*}}).AddArgument($($args[0]))

                    $Feature = ([PowerShell]::Create()).AddScript({param($DomControl)
...
Invoke-Command -cn $DomControl -ScriptBlock {Get-SmbServerConfiguration | Select EnableSMB1Protocol}}).AddArgument($($args[0]))

                    $HW = ([PowerShell]::Create()).AddScript({param($DomControl)
...
Invoke-Command -cn $DomControl -ScriptBlock {systeminfo /fo CSV | ConvertFrom-Csv}}).AddArgument($($args[0]))

                    $HWBkp = ([PowerShell]::Create()).AddScript({param($DomControl)
...
systeminfo /S $DomControl /fo CSV | ConvertFrom-Csv}).AddArgument($($args[0]))

                    $Backup = ([PowerShell]::Create()).AddScript({param($DomControl)
...
repadmin /showbackup $DomControl}).AddArgument($($args[0]))

                    $NTP1 = ([PowerShell]::Create()).AddScript({param($DomControl)
...
Invoke-Command -cn $DomControl -ScriptBlock {W32TM /query /status}}).AddArgument($($args[0]))

                    $NTP2 = ([PowerShell]::Create()).AddScript({param($DomControl)
...
Invoke-Command -cn $DomControl -ScriptBlock {W32TM /query /configuration}}).AddArgument($($args[0]))

                    $HotFix = ([PowerShell]::Create()).AddScript({param($DomControl)
...
Get-HotFix -ComputerName $DomControl | Sort-Object { [datetime]$_.InstalledOn },HotFixID -desc | Select-Object -First 1}).AddArgument($($args[0]))

                    $Proc = ([PowerShell]::Create()).AddScript({param($DomControl)
...
(Get-CimInstance -Class Win32_ComputerSystem -ComputerName $DomControl).NumberOfLogicalProcessors}).AddArgument($($args[0]))

                    $FreeSpace = ([PowerShell]::Create()).AddScript({param($DomControl)
...
(Get-Counter -counter "\LogicalDisk(*)\% Free Space" -ComputerName $DomControl).CounterSamples}).AddArgument($($args[0]))

                    $Spooler = ([PowerShell]::Create()).AddScript({param($DomControl)
...
Get-CimInstance -ClassName Win32_Service -Filter "Name = 'Spooler'" -Property State,StartMode -ComputerName $DomControl}).AddArgument($($args[0]))

                    $GPResult = ([PowerShell]::Create()).AddScript({param($DomControl)
...
Get-GPResultantSetOfPolicy -Computer $DomControl -ReportType Xml -Path ("C:\ADxRay\Hammer\RSOP_"+$DomControl+".xml")}).AddArgument($($args[0]))

                    $DNS = ([PowerShell]::Create()).AddScript({param($DomControl)
...
Get-DnsServer -ComputerName $DomControl}).AddArgument($($args[0]))

                    $ldapRR = ([PowerShell]::Create()).AddScript({param($DomControl,$Dom)
...
Get-DnsServerResourceRecord -ZoneName ('_msdcs.'+$Dom) -Name '_ldap._tcp.dc' -ComputerName $DomControl}).AddArgument($($args[0])).AddArgument($($args[1]))
...
Add-Content $report "               <p>This section is intended to give an overall view of the existing <strong>Active Directory Domains</strong>, as so as the <strong>Active Directory Objects</strong>, <strong>Active Directory Group Policy Objects</strong> and <strong>Active Directory SysVolâ€™s Content</strong>. <p>" 
Add-Content $report "           </section>"
...
Add-Content $report "                       <th width='10%' align='center' title='DES encryption uses a 56-bit key to encrypt the content and is now considered to be highly insecure. Hence, accounts that can use DES to authenticate to services are at significantly greater risk of having that accountâ€™s logon sequence decrypted and the account compromised.'>Use Kerberos DES</th>" 
Add-Content $report "                   </tr>" 
...
Add-Content $report "               <p>This section will give a detailed view of the Domain Controller's health. With tests and validations based on <strong>DCDiag</strong> tool and should be enough to give a deep status of Domain Controllerâ€™s overall heatlh. </p>" 
Add-Content $report "           </section>"
                ...
                Try 
                    {
                        ...
                        Add-Content $report "                       <td align=center>High</td>"
                        Add-Content $report "                       <td align=center>Validates if the registry key <strong>'HKEY_Local_Machine\System\CurrentControlSet\Services\Netlogon\Parameters\SysvolReady=1'</strong> exist. This registry has to exist with value '1' for the DCÂ´s SYSVOL to be advertised.</td>"
                        Add-Content $report "                   </tr>" 
                        Add-Content $report "                   <tr>"
                        ...
                        Add-Content $report "                       <td align=center>Medium</td>"
                        Add-Content $report "                       <td align=center>Validates if permissions are correctly set in this Domain Controller for all naming contexts. Those permissions directly affect replicationÂ´s health.</td>"
                        Add-Content $report "                   </tr>" 
                        Add-Content $report "                   <tr>"
...
Add-Content $report "               <p>This section will give a detailed view of the Domain Controller's Security. This inventory is based on MicrosoftÂ´s best practices and recommendations.</p>" 
Add-Content $report "           </section>"
...
Add-Content $report  "<p class='note'><font color=$TableFontOnError>Critical Security Events were found in this environment! Investigate further following MicrosoftÂ´s <a href='https://docs.microsoft.com/en-us/windows-server/identity/ad-ds/plan/appendix-l--events-to-monitor' target='_blank' rel='external'>Events to Monitor in Active Directory</a>. </font></p>" 
...
Add-Content $report "           </CENTER>"
Add-Content $report "           <CENTER>"
Add-Content $report "               <p class='note'>According to Microsoft: â€˜While seemingly harmless, any authenticated user can remotely connect to a domain controller print spooler service, and request an update on new print jobs. In addition, users can tell the domain controller to send the notification to the system with unconstrained delegation. These actions test the connection and expose the domain controller computer account credential (Print spooler is owned by SYSTEM).â€™. <a href=â€™https://docs.microsoft.com/en-us/azure-advanced-threat-protection/atp-cas-isp-print-spooler' target='_blank' rel='external'>Security assessment: Domain controllers with Print spooler service available</a>. For a deeper insight and better security view, monitor and protection of your domain controllerâ€™s environment give a look and perhaps a try on Azure Advanced Treat Protection (<a href='https://docs.microsoft.com/en-us/azure-advanced-threat-protection/what-is-atp' target='_blank' rel='external'>What is Azure Advanced Threat Protection?</a>).</p>" 
Add-Content $report "           </CENTER>"
...
Add-Content $report "           <CENTER>"
Add-Content $report "               <p class='note'>This analysis is based on the Resultant Set of Policies applied on those Domain Controllers versus MicrosoftÂ´s baseline security standards. Microsoft recommends the use of security baseline GPOs (<a href='https://techcommunity.microsoft.com/t5/microsoft-security-baselines/security-baseline-for-windows-10-version-21h2/ba-p/3042703' target='_blank' rel='external'>Security baseline for Windows 10, version 21H2</a>) in the environment, specially on Domain Controllers. Keep your environment protected with the lastest security baseline!</p>" 
Add-Content $report "           </CENTER>"
...
Add-Content $report "               </CENTER>"
Add-Content $report "               <p class='note'>Make sure that the software listed above are really necessary on those servers, in case you donâ€™t have a real need to keep them. Uninstall them from the Domain Controllers as soon as possible. </p>" 
