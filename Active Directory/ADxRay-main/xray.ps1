...
function HammerDC {
    ...
    Write-Progress -Activity 'Running Inventories' -Status "10% Complete." -CurrentOperation 'Triggering Domain Controller Inventory from workstation...'
    ...
    foreach ($DC in $Global:DCs) {
        while ($Jobs.Count -ge $MaxParallel) {
            $null = Wait-Job -Job $Jobs -Any -Timeout 30
            $Jobs = $Jobs | Where-Object { $_.State -eq 'Running' }
        }

        Add-Content $Global:ADxRayLog ((Get-Date -Format 'MM-dd-yyyy  HH:mm:ss') + " - Info - Starting Inventory for DC: " + $DC.Name)

        $job = Start-Job -Name "Inv_$($DC.Name)" -ScriptBlock {
            param($DCName, $Domain, $Cred)

            try {
                $Session = New-PSSession -ComputerName $DCName -Credential $Cred -ErrorAction Stop

                $GPResultPath = "C:\ADxRay\Hammer\RSOP_$DCName.xml"
                Invoke-Command -Session $Session -ScriptBlock {
                    param($Path)
                    Get-GPResultantSetOfPolicy -Computer $env:COMPUTERNAME -ReportType Xml -Path $Path
                } -ArgumentList $GPResultPath

                Remove-PSSession $Session
                return @{ DC = $DCName; Status = "Success"; Path = $GPResultPath }
            }
            catch {
                return @{ DC = $DCName; Status = "Failed"; Error = $_.Exception.Message }
            }

        } -ArgumentList $DC.Name, $DC.Domain, $Cred

        $Jobs += $job
    }

    foreach ($job in $Jobs) {
        if (Wait-Job -Job $job -Timeout $JobTimeout) {
            $result = Receive-Job -Job $job
            $log = "$($result.DC) - $($result.Status)"
            if ($result.Status -eq "Failed") {
                $log += " - Error: $($result.Error)"
            }
            Add-Content $Global:ADxRayLog ((Get-Date -Format 'MM-dd-yyyy  HH:mm:ss') + " - Result - " + $log)
        } else {
            Stop-Job -Job $job -Force
            Add-Content $Global:ADxRayLog ((Get-Date -Format 'MM-dd-yyyy  HH:mm:ss') + " - Timeout - Job for $($job.Name)")
        }
    ...
    Add-Content $Global:ADxRayLog ((Get-Date -Format 'MM-dd-yyyy  HH:mm:ss') + " - Info - All Inventories are completed")
}
