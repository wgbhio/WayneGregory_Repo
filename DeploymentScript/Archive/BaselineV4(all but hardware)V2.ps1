...
function Get-SiteSuffix {
  ...
  switch ($Region) {
    ...
    'IE' { return 'DB4' }
    'US' {
      if ($UsSite) { return $UsSite } else { return 'US' }
    }
...
function Convert-StructuredConfig {
  ...
  # ---- US / site suffixing & template naming (after normalization) ----
...
function Deploy-FromCLv2 {
  ...
  try {
    ...
    if ($config.AdditionalDisks -and ($config.AdditionalDisks -isnot [System.Array])) { $config.AdditionalDisks = @($config.AdditionalDisks) }
    ...
    # --- Transcript
    $deployLog = Join-Path "C:\Users\GrWay\OneDrive - Beazley Group\Documents\Scripts\DeploymentScript" ("VMDeploy-{0:yyyyMMdd-HHmmss}-{1}.log" -f (Get-Date), $config.VMName)
    ...
    # --- Content Library → ensure local, site-suffixed template ---------------------
...
function Ensure-LocalTemplateFromCL {
  ...
  $libName     = $Config.ContentLibraryName
  $libItemName = $Config.TemplateName
  $tplName     = $Config.LocalTemplateName
  $force       = [bool]$Config.ForceReplace

  Write-Host "Materializing local template '$tplName' (from CL item '$libItemName' in '$libName') ..."

  $existingTemplate = VMware.VimAutomation.Core\Get-Template -Name $tplName -ErrorAction SilentlyContinue
  if ($existingTemplate -and $force) {
    Write-Host "ForceReplace is set: removing existing template '$tplName' ..."
    VMware.VimAutomation.Core\Remove-Template -Template $existingTemplate -DeletePermanently -Confirm:$false
    $existingTemplate = $null
  }
  if ($existingTemplate) {
    Write-Host "Local template '$tplName' already exists. Skipping creation."
    return $existingTemplate
  }

  # Host/Datastore selection
  $clusterObj = VMware.VimAutomation.Core\Get-Cluster -Name $Config.Cluster -ErrorAction Stop
  $host = if ($Config.VMHost) { VMware.VimAutomation.Core\Get-VMHost -Name $Config.VMHost } else { ($clusterObj | VMware.VimAutomation.Core\Get-VMHost | Select-Object -First 1) }
  $ds   = $null
  if ($Config.DatastoreCluster) {
    $sp = VMware.VimAutomation.Core\Get-DatastoreCluster -Name $Config.DatastoreCluster -ErrorAction Stop
    $ds = Select-BestDatastoreFromCluster -ClusterOrStoragePod $sp
  } elseif ($Config.Datastore) {
    $ds = VMware.VimAutomation.Core\Get-Datastore -Name $Config.Datastore -ErrorAction Stop
  } else {
    $ds = VMware.VimAutomation.Core\Get-Datastore -RelatedObject $host | Sort-Object FreeSpaceGB -Descending | Select-Object -First 1
  }

  # Primary: deploy from Content Library item -> convert to template
  $seedVM = $null
  try {
    if (Get-Module -ListAvailable -Name VMware.VimAutomation.Content) {
      $cl = VMware.VimAutomation.Content\Get-ContentLibrary -Name $libName -ErrorAction Stop
      $cli = VMware.VimAutomation.Content\Get-ContentLibraryItem -Name $libItemName -ContentLibrary $cl -ErrorAction Stop
      $tempVmName = "__seed_{0}_{1}" -f $tplName, ([guid]::NewGuid().ToString('N').Substring(0,8))
      Write-Host "Deploying temporary VM '$tempVmName' from Content Library item ..."
      $seedVM = VMware.VimAutomation.Core\New-VM -Name $tempVmName -ContentLibraryItem $cli -VMHost $host -Datastore $ds -ErrorAction Stop
    }
  } catch {
    Write-Warning "CL deploy failed or item is not directly deployable. Will try local template fallback."
  }

  # Fallback: clone from unsuffixed local template with same name as CL item
  if (-not $seedVM) {
    $srcTemplate = VMware.VimAutomation.Core\Get-Template -Name $libItemName -ErrorAction SilentlyContinue
    if (-not $srcTemplate) {
      throw "Neither CL deploy nor local source template '$libItemName' is available to create '$tplName'."
    }
    $tempVmName = "__seed_{0}_{1}" -f $tplName, ([guid]::NewGuid().ToString('N').Substring(0,8))
    Write-Host "Cloning temporary VM '$tempVmName' from local template '$libItemName' ..."
    $seedVM = VMware.VimAutomation.Core\New-VM -Name $tempVmName -Template $srcTemplate -VMHost $host -Datastore $ds -ErrorAction Stop
  }
  ...
  if ($Config.Network) {
    try {
      ...
      $networkObj = $allPG | Where-Object { $_.Name -eq $Config.Network }
      if ($networkObj) {
        $nic = VMware.VimAutomation.Core\Get-NetworkAdapter -VM $seedVM | Select-Object -First 1
        if ($nic) {
          if ($networkObj -is [VMware.VimAutomation.Vds.Types.V1.VDPortgroup]) {
            VMware.VimAutomation.Core\Set-NetworkAdapter -NetworkAdapter $nic -PortGroup $networkObj -Confirm:$false | Out-Null
          } else {
            VMware.VimAutomation.Core\Set-NetworkAdapter -NetworkAdapter $nic -NetworkName $Config.Network -Confirm:$false | Out-Null
          }
        }
      }
    } catch {}
  ...
  Write-Host "Converting '$($seedVM.Name)' to local template '$tplName' ..."
  $template = VMware.VimAutomation.Core\Set-VM -VM $seedVM -ToTemplate -Name $tplName -ErrorAction Stop

  # Ensure template is in the desired folder if provided
  if ($Config.TemplatesFolderName) {
    try {
      $dc = Get-DatacenterForCluster -Cluster $clusterObj
      $tplFolder = Ensure-TemplatesFolderStrict -Datacenter $dc -FolderName $Config.TemplatesFolderName
      VMware.VimAutomation.Core\Move-Template -Template $template -Destination $tplFolder -Confirm:$false | Out-Null
    } catch {
      Write-Warning ("Template folder '{0}' not found; leaving template at current location." -f $Config.TemplatesFolderName)
    }
  }

  Write-Host ("Local template created: {0}" -f $template.Name)
  return $template
}
...
function New-VMFromLocalTemplate {
  ...
  $ds   = if ($Config.DatastoreCluster) { 
            ...
            VMware.VimAutomation.Core\Get-Datastore -RelatedObject $host | Sort-Object FreeSpaceGB -Descending | Select-Object -First 1 
          }
