...
<#
VM Deploy V2 – Clean Baseline
- Guided JSON builder (region/env/cluster/network/OU/template/HW + interactive extra disks)
- Refresh local template from Content Library, then deploy VM
- Additional data disks spread round-robin across PVSCSI 1..3 (system remains on SCSI 0)
- AD move happens LAST (no replication), with robust DC search and clear result
- Minimal VM Notes: "CR: <num> — deployed <timestamp>"
- Configurable waits: Options.ToolsWaitSeconds (default 90), Options.PostPowerOnDelay (default 15), AD.WaitForSeconds (default 180)
- Step logging via Write-Stamp + Invoke-Step; transcript to %TEMP%
#>
...
function Invoke-Step {
  ...
  try{ $r=& $Script; $sw.Stop(); Write-Stamp ("✓ {0} ({1:g})" -f $Name,$sw.Elapsed); return $r }
    ...
    if ($_.ScriptStackTrace){ Write-Host ("  • STACK: {0}" -f $_.ScriptStackTrace) -ForegroundColor DarkYellow }
    throw
  }
}
...
function AskInt { param([string]$Prompt,[int]$Default)
  while ($true){ $v = Ask -Prompt $Prompt -Default $Default; if ($v -as [int] -ne $null){ return [int]$v }; Write-Host "Please enter a whole number." -ForegroundColor Yellow }
}
...
# -------------------- JSON helpers --------------------
function Remove-JsonComments { param([Parameter(Mandatory)][string]$Text)
  $sb = New-Object System.Text.StringBuilder
  $inString=$false; $inLine=$false; $inBlock=$false; $escape=$false
  for ($i=0; $i -lt $Text.Length; $i++) {
    $ch = $Text[$i]; $nx = if ($i+1 -lt $Text.Length) { $Text[$i+1] } else { [char]0 }
...
function Read-Config { param([string]$Path)
...
function Resolve-RootFolder { param([Parameter(Mandatory)]$Datacenter,[Parameter(Mandatory)][string]$Name)
  ...
  $folder = Get-Folder -Location $vmRoot -Type VM -ErrorAction SilentlyContinue | Where-Object { $_.Name -eq $Name } | Select-Object -First 1
...
function Wait-ForADComputer {
  param([string]$Name,[string]$BaseDN,[int]$TimeoutSec=180,[System.Management.Automation.PSCredential]$Credential,[string]$Server)
  $elapsed=0
  while($elapsed -lt $TimeoutSec){
    try{
      ...
      $obj=Get-ADComputer @p; if($obj){return $obj}
    } catch{}
...
function Get-AdCredential {
  param([bool]$UseVCenterCreds,[System.Management.Automation.PSCredential]$VcCred,[string]$CredFile,[string]$DefaultCredFilePath,[switch]$PromptIfMissing)
...
function Get-AdDcCandidates {
  ...
  try {
    ...
    $all = Get-ADDomainController -Filter * -ErrorAction Stop | Where-Object { -not $_.IsReadOnly }
    if ($Region -eq 'US' -and $UsSite) {
      $siteName = if ($UsSite -eq 'HAW') { 'Hawthorne' } elseif ($UsSite -eq 'MAR') { 'Marlborough' } else { $null }
      if ($siteName) { $cands += ($all | Where-Object { $_.Site -eq $siteName } | Select-Object -ExpandProperty HostName) }
      $cands += ($all | Where-Object { $_.HostName -like 'USPRDC036*' } | Select-Object -ExpandProperty HostName)
      $cands += ($all | Where-Object { $_.HostName -like 'USPRDC*'   } | Select-Object -ExpandProperty HostName)
    } elseif ($Region -eq 'UK') {
      $cands += ($all | Where-Object { $_.HostName -like 'UKPRDC*' } | Select-Object -ExpandProperty HostName)
    } elseif ($Region -eq 'IE') {
      $cands += ($all | Where-Object { $_.HostName -like 'IEPRDC*' } | Select-Object -ExpandProperty HostName)
    }
    $cands += ($all | Select-Object -ExpandProperty HostName)
...
function New-VM_FilteringOvfWarning { param([hashtable]$Params)
  $warn=@(); $vm = New-VM @Params -WarningAction SilentlyContinue -WarningVariable +warn
  if ($warn) { $ignore=[regex]'com\.vmware\.vcenter\.ovf\.ovf_warning.*target_datastore.*sdrs'; foreach($w in $warn){ $s=[string]$w; if(-not $ignore.IsMatch($s)){ Write-Warning $s } } }
  return $vm
}
function Upgrade-VMHardwareSafely { param([Parameter(Mandatory)]$VM,[switch]$RePowerOn)
  try { Set-VM -VM $VM -UpgradeHardware -Confirm:$false -ErrorAction Stop | Out-Null; Write-Stamp "Hardware upgrade request issued." }
  catch {
    $msg="$($_.Exception.Message)"
    if ($msg -match 'current state.*Powered on') {
      Write-Stamp "Powering off to upgrade hardware…"; Stop-VM -VM $VM -Confirm:$false -ErrorAction Stop | Out-Null
      do { Start-Sleep 2 } while ((Get-VM -Id $VM.Id).PowerState -ne 'PoweredOff')
      Set-VM -VM $VM -UpgradeHardware -Confirm:$false -ErrorAction Stop | Out-Null; Write-Stamp "Hardware upgraded while powered off."
...
# -------------------- Low-level SCSI helpers (API) --------------------
function Get-ScsiSnapshot { param([Parameter(Mandatory)]$VM)
  $v = Get-View -Id $VM.Id
  $list=@()
...
function Add-PvScsiControllerApi { param([Parameter(Mandatory)]$VM,[Parameter(Mandatory)][int]$BusNumber)
  $vmView = Get-View -Id $VM.Id
  $exists = $vmView.Config.Hardware.Device | Where-Object { $_ -is [VMware.Vim.ParaVirtualSCSIController] -and $_.BusNumber -eq $BusNumber }
...
function Get-NextScsiUnitNumber { param([Parameter(Mandatory)]$VmView,[Parameter(Mandatory)][int]$ControllerKey)
  $used=@()
  foreach($d in $VmView.Config.Hardware.Device){
    if ($d -is [VMware.Vim.VirtualDisk] -and $d.ControllerKey -eq $ControllerKey -and $d.UnitNumber -ne $null){ $used += [int]$d.UnitNumber }
  }
...
function Move-HardDiskToBusApi { param([Parameter(Mandatory)]$VM,[Parameter(Mandatory)]$HardDisk,[Parameter(Mandatory)][int]$TargetBus)
  $vmView = Get-View -Id $VM.Id
  if ($TargetBus -ge 1) {
    $have = $vmView.Config.Hardware.Device | Where-Object { $_ -is [VMware.Vim.ParaVirtualSCSIController] -and $_.BusNumber -eq $TargetBus } | Select-Object -First 1
    if (-not $have) { Add-PvScsiControllerApi -VM $VM -BusNumber $TargetBus | Out-Null; Start-Sleep -Milliseconds 700; $vmView = Get-View -Id $VM.Id }
  }
  $controllers=@{}
  foreach($c in $vmView.Config.Hardware.Device){ if ($c -is [VMware.Vim.VirtualSCSIController]) { $controllers[$c.BusNumber]=$c } }
  if (-not $controllers.ContainsKey($TargetBus)) { throw "No SCSI controller on bus $TargetBus." }
  $ctrl   = $controllers[$TargetBus]
  $diskDev= $vmView.Config.Hardware.Device | Where-Object { $_ -is [VMware.Vim.VirtualDisk] -and $_.Key -eq $HardDisk.ExtensionData.Key } | Select-Object -First 1
...
function Build-DeployConfig {
  ...
  $ADServerMap = @{ 'UK'='UKPRDC011.bfl.local'; 'IE'='IEPRDC010.bfl.local'; 'US|HAW'='USPRDC036.bfl.local'; 'US|MAR'='USPRDC036.bfl.local' }
  ...
  elseif ($region -eq 'IE') { $cluster='DB4Compute'; $dsCluster='DB4_DatastoreCluster' }
    ...
    if ($row.Site) { $usSite=$row.Site } elseif ($picked -eq 'US Stretched') { $usSite = AskChoice -Prompt 'Select US site for OU' -Choices @('HAW','MAR') -DefaultIndex 0 }
  }
  ...
  # OS/Template
  $tmplChoice   = AskChoice -Prompt 'Choose OS / Template' -Choices @('Windows 2022','Windows 2019','Linux CentOS 7','Linux RHEL 7','Linux RHEL 9','Linux SLES 15') -DefaultIndex 0
  ...
  # Additional disks (interactive)
  $addCount = AskInt -Prompt 'How many additional data disks?' -Default 0
  $additionalDisks = @()
  for ($i=1; $i -le $addCount; $i++){ $additionalDisks += (AskInt -Prompt ("  Size of additional disk #{0} (GB)" -f $i) -Default 50) }

  # VM Name + CR
  do { $vmName1 = Ask -Prompt 'VM Name'; $vmName2 = Ask -Prompt 'Confirm VM Name'; if ($vmName1 -ne $vmName2){ Write-Host 'Names did not match. Try again.' -ForegroundColor Yellow } } until ($vmName1 -eq $vmName2)
  $vmName = $vmName1
  $changeNum = Ask -Prompt 'Change Number (goes into VM Notes)' -AllowEmpty

  # Compose JSON
  $vmw = [ordered]@{
    VCSA=$vcsa; Cluster=$cluster; VMHost=$null; VMFolder=($(if ($env -eq 'PROD'){'Production'}else{'Development'})); TemplatesFolderName='Templates'; Network=$network;
  ...
  if ($adOU) {
    $out.AD = [ordered]@{ TargetOU=$adOU; Server=$adServer; UseVCenterCreds=$true; WaitForSeconds=180 }
  }
...
function Convert-StructuredConfig {
  ...
  [pscustomobject]@{
    ...
    CPU=$hw.CPU; MemoryGB=$hw.MemoryGB; DiskGB=$hw.DiskGB; AdditionalDiskGB=$hw.AdditionalDiskGB; AdditionalDisks=$hw.AdditionalDisks; Sockets=$hw.Sockets; CoresPerSocket=$hw.CoresPerSocket
    ADTargetOU=$ad.TargetOU; ADServer=$ad.Server; ADUseVCenterCreds=Pick @($ad.UseVCenterCreds,$true); ADWaitForSeconds=Pick @($ad.WaitForSeconds,180); ADCredFile=$ad.CredFile; ADUnmanagedOU=$ad.UnmanagedOU
    ...
    ToolsWaitSeconds=Pick @($opt.ToolsWaitSeconds,90); PostPowerOnDelay=Pick @($opt.PostPowerOnDelay,15)
...
function Deploy-FromCLv2 {
  ...
  # local helper for AD move (no replication)
  function Invoke-AdMove {
    param([Parameter(Mandatory)]$Config,[Parameter(Mandatory)]$CredPathRoot,[Parameter(Mandatory)]$SafeVcsaName,[Parameter(Mandatory)]$VcCred,[Parameter(Mandatory)]$Vm,[Parameter(Mandatory)]$RawConfig)
    Write-Stamp "AD BLOCK: starting (final step)"
    if (-not $Config.ADTargetOU) { $r='No ADTargetOU in config'; Write-Warning $r; return [pscustomobject]@{Result='SKIPPED';Reason=$r} }
    Write-Stamp ("AD BLOCK: TargetOU = {0}" -f $Config.ADTargetOU)

    try { Import-Module ActiveDirectory -ErrorAction Stop; Write-Stamp "AD BLOCK: ActiveDirectory module imported" }
    catch { $r = "ActiveDirectory module not available: $($_.Exception.Message)"; Write-Warning $r; return [pscustomobject]@{Result='SKIPPED';Reason=$r} }
    ...
    try { $adCred = Get-AdCredential -UseVCenterCreds:$Config.ADUseVCenterCreds -VcCred:$VcCred -CredFile:$Config.ADCredFile -DefaultCredFilePath:$DefaultAdCredFile -PromptIfMissing; Write-Stamp ("AD BLOCK: Using credential identity = {0}" -f ($adCred ? $adCred.UserName : '<null>')) }
    catch { $r="Could not resolve AD credentials: $($_.Exception.Message)"; Write-Warning $r; return [pscustomobject]@{Result='SKIPPED';Reason=$r} }

    $searchBase = if ($RawConfig.AD -and $RawConfig.AD.UnmanagedOU) { $RawConfig.AD.UnmanagedOU } elseif ($Config.ADUnmanagedOU) { $Config.ADUnmanagedOU } else { Get-BaseDNFromTargetOU -TargetOU $Config.ADTargetOU }
    Write-Stamp ("AD BLOCK: SearchBase = {0}" -f $searchBase)
    Write-Stamp ("AD BLOCK: Preferred DC = {0}" -f ($(if($adServer){$adServer}else{'<none>'})))
    ...
    try { $p=@{ Identity=$Config.ADTargetOU; ErrorAction='Stop' }; if($adServer){$p.Server=$adServer}; if($adCred){$p.Credential=$adCred}; Get-ADOrganizationalUnit @p | Out-Null; Write-Stamp "AD BLOCK: Target OU validated on preferred DC (or default)" } catch { Write-Warning ("AD BLOCK: Target OU validation on preferred DC failed: {0}" -f $_.Exception.Message) }
    ...
    foreach($dc in $dcList){
      ...
      try { $adComp = Wait-ForADComputer -Name $Config.VMName -BaseDN $searchBase -TimeoutSec $timeoutPerDc -Credential $adCred -Server $dc } catch { Write-Warning ("AD BLOCK: Search error on {0}: {1}" -f $dc,$_.Exception.Message) }
    ...
    if (-not $adComp) {
      try {
        ...
        if ($guestName -and $guestName -ne $Config.VMName) {
          ...
          foreach($dc in $dcList){
            try { $adComp = Wait-ForADComputer -Name $guestName -BaseDN $searchBase -TimeoutSec ([Math]::Min(60,$timeoutPerDc)) -Credential $adCred -Server $dc } catch { Write-Warning ("AD BLOCK: Retry search error on {0}: {1}" -f $dc,$_.Exception.Message) }
            if ($adComp) { Write-Stamp ("AD BLOCK: Found (guest name) on {0}" -f $dc); break }
          }
        } else { Write-Stamp "AD BLOCK: No distinct guest hostname available for retry." }
      } catch { Write-Warning ("AD BLOCK: Guest lookup failed: {0}" -f $_.Exception.Message) }
    }
    if (-not $adComp) { $r="Computer not found within $timeoutTotal s under '$searchBase' across DCs: $($dcList -join ', ')"; Write-Warning $r; return [pscustomobject]@{Result='NOTFOUND';Reason=$r} }

    $moveServer = if ($adServer) { $adServer } else { $dcList | Select-Object -First 1 }
    try {
      Invoke-Step -Name ("Move-ADObject to target OU on {0}" -f ($(if($moveServer){$moveServer}else{'<auto>'}))) -Script {
    ...
    } catch { $r="Move-ADObject error: $($_.Exception.Message)"; Write-Warning $r; if ($_.Exception){ Write-Host ("  • EXCEPTION: {0}" -f $_.Exception.ToString()) -ForegroundColor DarkYellow }; return [pscustomobject]@{ Result='FAILED'; Reason=$r } }
  }
  ...
  try {
    ...
    foreach ($key in @('VCSA','Cluster','Network','ContentLibraryName','TemplateName','Folder')) { if (-not $config.$key) { throw "Missing required config field: '$key'." } }
    ...
    $cred=$null
    if (Test-Path $CredFile) { try { $cred=Import-Clixml -Path $CredFile; if (-not ($cred -and $cred.UserName)) { $cred=$null } else { [void]$cred.GetNetworkCredential().Password } } catch { $cred=$null } }
    ...
    $templatesFolder= Ensure-TemplatesFolderStrict -Datacenter $dc -FolderName ($(if ($config.TemplatesFolderName){$config.TemplatesFolderName}else{'Templates'}))
    ...
    if ($config.DatastoreCluster) {
      ...
      if ($dsCluster) { Write-Host "Using datastore cluster '$($dsCluster.Name)'; selecting datastore with most free space..."; $datastoreObj = Select-BestDatastoreFromCluster -Cluster $dsCluster; Write-Host "Selected datastore: $($datastoreObj.Name)" }
    }
    ...
    $usePortGroup=$false; try { $usePortGroup = ($networkObj -is [VMware.VimAutomation.Vds.Types.V1.VDPortgroup]) -or ($networkObj.GetType().Name -eq 'VDPortgroup') } catch {}
    ...
    $ovfCfg=$null
    try { $ovfCfg = Get-OvfConfiguration -ContentLibraryItem $clItem -Target $clusterObj -ErrorAction Stop; Write-Host 'Detected OVF/OVA; mapping networks via OVF configuration...' } catch { Write-Host 'Detected VM template CL item; setting network parameter...' }
    if ($ovfCfg){
    ...
    Write-Host ("Converting '{0}' to a local vSphere Template..." -f $localTemplateName)
    ...
    if (Get-VM -Name $config.VMName -ErrorAction SilentlyContinue){ throw "A VM named '$($config.VMName)' already exists." }
    $oscSpec=$null; if ($config.CustomizationSpec){ $oscSpec=Get-OSCustomizationSpec -Name $config.CustomizationSpec -ErrorAction SilentlyContinue; if (-not $oscSpec){ Write-Warning "OS Customization Spec '$($config.CustomizationSpec)' not found; continuing without it." } }
    Write-Host ("`nDeploying VM '{0}' from local template '{1}'..." -f $config.VMName,$localTemplate.Name)
    ...
    # NIC set
    try{ $firstNic=Get-NetworkAdapter -VM $newVM | Select-Object -First 1
         if($firstNic){ if($usePortGroup){ Set-NetworkAdapter -NetworkAdapter $firstNic -PortGroup $networkObj -Confirm:$false | Out-Null } else { Set-NetworkAdapter -NetworkAdapter $firstNic -NetworkName $config.Network -Confirm:$false | Out-Null }
    ...
    if ($config.DiskGB){ $sysDisk=Get-HardDisk -VM $newVM | Select-Object -First 1; if($sysDisk -and $config.DiskGB -gt $sysDisk.CapacityGB){ Set-HardDisk -HardDisk $sysDisk -CapacityGB $config.DiskGB -Confirm:$false -ErrorAction SilentlyContinue | Out-Null } }

    # Additional disks: create only needed PVSCSI buses (1..min(3, count)), add/move per round-robin
    $additionalSizes = @()
    if ($rawConfig.VMware -and $rawConfig.VMware.Hardware -and $rawConfig.VMware.Hardware.AdditionalDisks) { $additionalSizes = @($rawConfig.VMware.Hardware.AdditionalDisks | ForEach-Object { [int]$_ }) }
    elseif ($config.AdditionalDisks) { $additionalSizes = @($config.AdditionalDisks | ForEach-Object { [int]$_ }) }
    elseif ($config.AdditionalDiskGB -and [int]$config.AdditionalDiskGB -gt 0) { $additionalSizes = @([int]$config.AdditionalDiskGB) }

    $buses=@()
    $n=[Math]::Max(0,$additionalSizes.Count)
    if ($n -gt 0) { $max=[Math]::Min(3,$n); for($b=1;$b -le $max;$b++){ $buses+=$b } }
    ...
    if ($additionalSizes.Count -gt 0) {
      for ($i=0; $i -lt $additionalSizes.Count; $i++) {
        ...
        try { Move-HardDiskToBusApi -VM $newVM -HardDisk $hd -TargetBus $bus } catch { Write-Host ("WARN: Move disk to SCSI{0} failed: {1}" -f $bus,$_.Exception.Message) -ForegroundColor Yellow }
        Write-Stamp ("Added data disk #{0} ({1} GB) on SCSI{2}" -f ($i+1), $size, $bus)
        Start-Sleep -Milliseconds 300
      }
    } else { Write-Stamp "No additional disk sizes provided." }

    # Minimal Notes
    if ($config.ChangeRequestNumber){ $note = "CR: $($config.ChangeRequestNumber) — deployed $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')"; try { Set-VM -VM $newVM -Notes $note -Confirm:$false | Out-Null; Write-Stamp "Notes set: $note" } catch { Write-Warning "Failed to set Notes: $($_.Exception.Message)" } }

    # Tools upgrade policy (if supported)
    try{ $cmd = Get-Command Set-VM -ErrorAction SilentlyContinue; if ($cmd -and $cmd.Parameters.ContainsKey('ToolsUpgradePolicy')) { Set-VM -VM $newVM -ToolsUpgradePolicy UpgradeAtPowerCycle -Confirm:$false | Out-Null; $view=Get-View -Id $newVM.Id; Write-Stamp ("Tools policy now: {0}" -f $view.Config.Tools.ToolsUpgradePolicy) } else { Write-Stamp "Skipping ToolsUpgradePolicy (parameter not supported in this PowerCLI)." } }
    catch { Write-Warning ("Set-VM ToolsUpgradePolicy failed: {0}" -f $_.Exception.Message) }

    # Power on
    if ($config.PowerOn){ Invoke-Step -Name "Power on VM" -Script { Start-VM -VM $newVM -Confirm:$false | Out-Null }; if ($config.PostPowerOnDelay -gt 0) { Write-Stamp ("Sleeping {0}s post power-on…" -f $config.PostPowerOnDelay); Start-Sleep -Seconds ([int]$config.PostPowerOnDelay) } }

    # Tools wait + optional tools update + HW upgrade
    if ($config.PowerOn){
      $tw = if ($config.ToolsWaitSeconds) { [int]$config.ToolsWaitSeconds } else { 90 }
      $status = $null
      try { $status = Invoke-Step -Name ("Wait for VMware Tools healthy ({0}s)" -f $tw) -Script { Wait-ToolsHealthy -VM $newVM -TimeoutSec $tw } } catch { Write-Warning ("Wait-ToolsHealthy threw: {0}" -f $_.Exception.Message) }
      ...
      try{
        ...
        if ($tools -in 'toolsOld','toolsSupportedOld','toolsOk') { try { Update-Tools -VM $newVM -NoReboot -ErrorAction SilentlyContinue | Out-Null; Write-Stamp 'Triggered VMware Tools upgrade.' } catch {}; $status2 = Wait-ToolsHealthy -VM $newVM -TimeoutSec $tw; Write-Stamp ("Post-upgrade VMware Tools status: {0}" -f ($status2 ?? 'unknown')) } else { Write-Stamp ("Skipping Update-Tools; Tools status is '{0}'" -f ($tools ?? 'unknown')) }
      } catch {}
    }
    try { Upgrade-VMHardwareSafely -VM $newVM -RePowerOn:($config.PowerOn) } catch { Write-Warning ("Hardware upgrade step failed: {0}" -f $_.Exception.Message) }

    # AD MOVE (last, no replication)
    ...
    # Final status
    try{ $v=Get-View -Id $newVM.Id; $v.UpdateViewData(); [void](Get-VM -Id $newVM.Id); $guest = Get-VMGuest -VM $newVM -ErrorAction SilentlyContinue; $toolsState = if ($guest) { $guest.ToolsStatus } else { 'unknown' }; $hwVer = (Get-VM -Id $newVM.Id).Version; Write-Stamp ("Final status → Tools: {0}; HW: {1}; Power: {2}" -f $toolsState,$hwVer,(Get-VM -Id $newVM.Id).PowerState) } catch { Write-Warning "View/status refresh failed: $($_.Exception.Message)" }
    $powerMsg = if ($config.PowerOn){ 'powered on; ' } else { 'not powered on; ' }
    Write-Host ("✅ VM '{0}' deployed. Extra disks placed round-robin on PVSCSI bus(es): {1}; {2}AD move attempted if provided." -f $config.VMName, ($(if ($buses){($buses | Sort-Object) -join ','} else {'<none>'})), $powerMsg)
