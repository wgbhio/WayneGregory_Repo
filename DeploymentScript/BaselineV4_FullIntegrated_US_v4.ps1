...
function Invoke-ApplyCohesityZertoTags {
  <#
    .SYNOPSIS
      Apply Cohesity (balanced) and Zerto tags to a VM, bound to an explicit vCenter connection.

    .NOTES
      - Requires: VMware.PowerCLI (or equivalent) with Tagging cmdlets available.
      - Fixes:
        * Accepts an explicit -Server (VIServer) instead of relying on $VM.VIServer.
        * Skips Cohesity on DV hosts (per latest requirement).
        * Handles single-cardinality tag replacement safely.
        * Robust site inference for Zerto; US skipped by design.
  #>
  [CmdletBinding()]
  param(
    [Parameter(Mandatory)]
  ...
  function Choose-LeastLoadedTag($Counts,[string[]]$TagOrder){
    # Stable sort: by count asc, then by original order
  ...
  function Assign-Tag-Safely{
    ...
    # Find any existing tag assignment in this category
  ...
  function Get-VM-SiteForZerto([object]$VM,[string]$Region,[object]$Server){
    # Prefer cluster name heuristics; fallback to region code
  ...
  switch ($site) {
    ...
    'US' {
      Write-Stamp "[Tags] Zerto: US site — skipped."
    }
    default {
      Write-Stamp "[Tags] Zerto: site not inferred — skipped."
    }
  }
}
...
function Invoke-ADPlacement {
  ...
  param(
    ...
    [Parameter()][switch]$ForceSync,
    [Parameter()][string]$PostSyncPeers
  )
  try {
    # Only Windows
    ...
    $creds = $null
    if ($UseVCenterCreds) {
      # There's no secure way to reuse the vCenter login here; prompt instead
      $creds = $null
    }
    if (-not $creds) {
      if ($ADCredFile -and (Test-Path $ADCredFile)) {
        try { $creds = Import-Clixml -Path $ADCredFile } catch {}
      }
    }
    if (-not $creds) {
      $creds = Get-Credential -Message "[AD] Enter domain creds for join/move (e.g. BFL\\admin)"
    }

    # Create a session and join domain if needed
    $sess = $null
    try {
      $sess = New-PSSession -VM $VM -ErrorAction Stop
    } catch {
      # Fallback to hostname WinRM if integration is not available
    ...
    Invoke-Command -Session $sess -ScriptBlock {
      ...
      function Get-DomainJoined {
        try { (Get-WmiObject Win32_ComputerSystem).PartOfDomain } catch { $false }
      }
      $needJoin = $true
      $joined = Get-DomainJoined
      if ($joined) { $needJoin = $false }
      if ($needJoin) {
        try {
    ...
    # OU move from management host (requires RSAT ActiveDirectory)
    if ($TargetOU) {
      try {
    ...
    # Optional replication sync
    if ($ForceSync) {
      try {
        repadmin /syncall /AdeP
        if ($PostSyncPeers) {
          foreach ($p in ($PostSyncPeers -split '[,; ]+')) { if ($p) { repadmin /syncall $p /AdeP } }
        }
      } catch {}
    }
  } catch {
...
function Install-CS-And-SCOM-IfNeeded {
  ...
  param(
    ...
    [Parameter()][string]$SCOM_MGMT_SVR = "UKPRAP183.bfl.local",
    [Parameter()][pscredential]$Credential
  )
  try {
    $isPR = ($VM.Name -match 'PR')
    $target = $VM.Name

    if (-not (Test-WSMan -ComputerName $target -ErrorAction SilentlyContinue)) {
      Write-Stamp ("[Installers] {0}: WSMan unreachable, skipping." -f $target); return
    }

    if (-not $Credential) { $Credential = Get-Credential -Message "[Installers] Creds for $target" }
    ...
    if (-not (Test-Path $LocalCS)) { Write-Warning "[Installers] CrowdStrike not found at $LocalCS"; return }
    ...
    try {
      ...
      if ($needCopyCS) { Copy-Item -ToSession $sess -Path $LocalCS -Destination $Remote_CS_Exe -Force }

      # Copy SCOM only on PR servers if local MSI exists
      if ($isPR -and (Test-Path $LocalSCOM)) {
        $needCopySCOM = Invoke-Command -Session $sess -ScriptBlock { param($p) -not (Test-Path $p) } -ArgumentList $Remote_SCOM_MSI
        if ($needCopySCOM) { Copy-Item -ToSession $sess -Path $LocalSCOM -Destination $Remote_SCOM_MSI -Force }
      }
      ...
      # Install SCOM (PR only)
      if ($isPR -and (Test-Path $LocalSCOM)) {
        $scom = Invoke-Command -Session $sess -ScriptBlock {
          param($msi,$mg,$ms)
          $hs = Get-Service -Name HealthService -ErrorAction SilentlyContinue
          if ($hs) { return "SCOM already ($($hs.Status))" }
          if (-not (Test-Path $msi)) { return "SCOM skipped (MSI missing)" }
          $props = @(
          ...
          if ($proc.ExitCode -ne 0) { return "SCOM install failed ($($proc.ExitCode))" }
          ...
          if ($hs2) { return "SCOM installed ($($hs2.Status))" }
          return "SCOM installed (svc not detected yet)"
        } -ArgumentList $Remote_SCOM_MSI,$SCOM_MG,$SCOM_MGMT_SVR
        Write-Stamp ("[Installers] {0}: {1}" -f $target,$scom)
      } else {
        if (-not $isPR) { Write-Stamp ("[Installers] {0}: SCOM skipped (not PR server)." -f $target) }
        if ($isPR -and -not (Test-Path $LocalSCOM)) { Write-Stamp ("[Installers] {0}: SCOM skipped (local MSI missing)." -f $target) }
      }
    } finally {
...
function Deploy-FromCLv2 {
  ...
  try {
    ...
    if ($config.AdditionalDisks -and $config.AdditionalDisks.Count -gt 0) {
      foreach($sz in $config.AdditionalDisks){
        VMware.VimAutomation.Core\New-HardDisk -VM $newVM -CapacityGB ([int]$sz) -Confirm:$false -ErrorAction Stop | Out-Null
      }
    }
    ...
    # Tags first (ensures vCenter-bound context), then AD, then installers
    try {
      Invoke-ApplyCohesityZertoTags -VM $newVM -ErrorAction Stop
    } catch {
      Write-Warning ("Post-deploy tagging failed: {0}" -f $_.Exception.Message)
    }

    if ($config.PSObject.Properties['AD']) {
      try {
        Invoke-ADPlacement -VM $newVM -TargetOU $config.ADTargetOU -ADServer $config.ADServer -UseVCenterCreds:($config.ADUseVCenterCreds) -ADCredFile $config.ADCredFile -WaitForSeconds ([int]$config.ADWaitForSeconds) -ForceSync:($config.ADForceSync) -PostSyncPeers $config.ADPostSyncPeers
      } catch {
        Write-Warning ("[AD] Placement step failed: {0}" -f $_.Exception.Message)
      }
    }
    ...
    if ($config.PSObject.Properties['PostDeploy'] -and $config.PostDeploy.EnableInstallers) {
      try {
        Install-CS-And-SCOM-If-Needed -VM $newVM
      } catch {
