...
$SCOMSvr   = 'UKPRAP184'        # SCOM management server (must have OpsMgr module in WinPS 5.1)
...
function Log-Message {
    ...
    for ($i=1; $i -le $maxAttempts; $i++) {
        try {
            ...
            if ($i -eq $maxAttempts) {
                Write-Warning "Log file locked; writing to console only: $($_.Exception.Message)"
            }
...
function Get-ADLookupName {
    ...
    $name = $name -replace '\s+', ''            # drop spaces
    $name = $name -replace '-CHG\d+$',''        # drop -CHG suffix
    return $name.Trim()
}
...
function Invoke-Remote {
    ...
    # Force Windows PowerShell 5.1 endpoint on the remote side
    try {
...
function Invoke-ADResolveComputerDN {
    param(
        [Parameter(Mandatory=$true)][string]$ComputerIdentityOrName
    )
...
function Invoke-ADGetWUGroups {
    ...
    $sb = {
        ...
        if (-not $wuGroups -or $wuGroups.Count -eq 0) { return @() }
...
function Test-SCOMRegistration {
    ...
    $shortName = $HostnameFQDN.Split('.')[0]
    $baseName  = $shortName -replace '-CHG\d+$',''
    ...
    try {
        ...
        $scomStatus = Invoke-Remote -ComputerName $SCOMServer -ScriptBlock {
            ...
            # Ensure MG connection in the remote session
            ...
            # Guard: is Uninstall-SCOMAgent visible in THIS remote session?
            $uninstallCmd = Get-Command Uninstall-SCOMAgent -ErrorAction SilentlyContinue
            if (-not $uninstallCmd) {
                return @{
                    IsRegistered = $true
                    Removed      = $false
                    Error        = "Uninstall-SCOMAgent not available on $env:COMPUTERNAME in remote session. Ensure SCOM PowerShell components are installed."
            ...
            if ($doRemove) {
                ...
                # TEST mode: just report registration
                return @{
                    IsRegistered = $true
                    Removed      = $false
                    HealthState  = $agent.HealthState
    ...
    try {
        ...
        $evt = Get-VIEvent -Entity $vm -MaxSamples 400 |
            Where-Object {
                $_.GetType().Name -in @(
                    'VmPoweredOffEvent',
                    'VmPoweredOffByHostEvent',
                    'VmPoweredOffUsingGuestOperationsEvent',
                    'TaskEvent'
                ) -or
        ...
        if ($evt) {
            ...
            if ($vm.PowerState -eq 'PoweredOff') {
                [PSCustomObject]@{
                    ...
                    ShutdownDate      = $null
                    DaysSinceShutdown = $null
                    Error             = 'Powered off but no event found'
                }
            } else {
                [PSCustomObject]@{
                    VMName            = $VMName
                    ShutdownDate      = $null
                    DaysSinceShutdown = $null
                    Error             = 'No power-off event found'
                }
            }
        }
    } catch {
...
if ($TargetHosts.Count -gt 0) {
    Log-Message ("Targets loaded from change list: {0}" -f ($TargetHosts -join ', '))
}
...
foreach ($t in $TargetHosts) {
    ...
    foreach ($m in $vmMatches) {
        ...
        $dsNames = ''
        try {
            $dsNames = [string]::Join(",", (Get-Datastore -Id $m.DatastoreIdList | Select-Object -ExpandProperty Name))
        } catch {}
...
function Step1-ConnectivityAndAge($vms) {
    ...
    foreach ($vm in $vms) {
        ...
        try {
            ...
            $row.ProcessingErrors = "Step1 error: $_"
            Log-Message "Step1 error on ${($vm.VMName)}: $_" 'ERROR'
        }
        $rows += $row
    }
...
function Step2-SCOM($rows) {
    foreach ($row in $rows) {
        try {
            ...
            $row.ProcessingErrors += " Step2 error: $_"
            Log-Message "Step2 error on ${($row.VMName)}: $_" 'ERROR'
        }
    }
    return $rows
}
...
function Step4-AD_DNS_Groups($rows) {
    foreach ($row in $rows) {
        try {
            ...
            $row.ProcessingErrors += " Step4 error: $_"
            Log-Message "Step4 error on ${($row.VMName)}: $_" 'ERROR'
        }
    }
    return $rows
}
...
function Step5-FindOlder30d($rows) {
    ...
    if ($older.Count -gt 0) {
        ...
        Write-Host "No PoweredOff VMs older than 30 days were found based on notes." -ForegroundColor DarkYellow
    }
    return $rows
}
...
function Step6-RemoveVMs($rows) {
    ...
    if ($older.Count -eq 0) {
        Write-Host "No VMs qualify for deletion (>30d & PoweredOff)." -ForegroundColor Yellow
        return $rows
    }
    if ($Test) {
        foreach ($r in $older) {
            Log-Message "TEST: Would Remove-VM -DeletePermanently ${($r.VMName)}"
        }
    } else {
        ...
        if ($confirm -eq 'DELETE') {
            foreach ($r in $older) {
                try {
                    Remove-VM -VM (Get-VM -Name $r.VMName) -DeletePermanently -Confirm:$false | Out-Null
                    Log-Message "ACTION: Deleted VM ${($r.VMName)}"
                    $r.ActionsNeeded += ' | ACTION: VM deleted'
                }
                catch {
                    Log-Message "ERROR: Failed to delete VM ${($r.VMName)}: $_" 'ERROR'
                }
            }
        } else {
...
function Export-Results($rows) {
    $rows | Export-Csv -Path $CsvResults -NoTypeInformation
    Log-Message "Results exported to: $CsvResults"
}
...
while ($true) {
    ...
    switch ($opt.ToUpper()) {
        ...
        '2' {
            if (-not $rows -or $rows.Count -eq 0) { $rows = Step1-ConnectivityAndAge $vmResults }
            $rows = Step2-SCOM $rows
        }
        '3' {
            if (-not $rows -or $rows.Count -eq 0) { $rows = Step1-ConnectivityAndAge $vmResults }
            $rows = Step3-Shutdown $rows
        }
        '4' {
            if (-not $rows -or $rows.Count -eq 0) { $rows = Step1-ConnectivityAndAge $vmResults }
            $rows = Step4-AD_DNS_Groups $rows
        }
        '5' {
            if (-not $rows -or $rows.Count -eq 0) { $rows = Step1-ConnectivityAndAge $vmResults }
            $rows = Step5-FindOlder30d $rows
        }
        '6' {
            if (-not $rows -or $rows.Count -eq 0) { $rows = Step1-ConnectivityAndAge $vmResults }
            $rows = Step6-RemoveVMs $rows
        }
        '7' {
            if (-not $rows -or $rows.Count -eq 0) { $rows = Step1-ConnectivityAndAge $vmResults }
            Export-Results $rows
